export class MDate {
    ...
    getDateInDMY(date) {
        ...
        day = this.formateNbWithZeroBeforeIfNeeded(day);
        let month = date.getMonth() + 1;
        month = this.formateNbWithZeroBeforeIfNeeded(month);
        const year = date.getFullYear();
        return `${year}-${month}-${day}`;
    }

    formateNbWithZeroBeforeIfNeeded(nb) {
    ...
    getDateFromDateDMY(dateDMY) {
        const realDate = new Date(dateDMY);
    ...
    getCardsRIP(cards) {
        ...
        for (let i = 0; i < cards.length; i++) {
            const cardDate = new Date(cards[i].date);
